#Set resource keyword name
$name = Read-Host -Prompt "Enter the name of the new resource - max 5 characters"
$subscriptionName = read-host -Prompt "Enter the name of the Azure subscription to use"

#Initiate resourc creation
...
#check if resource group exist - else create
    if (Get-AzResourceGroup -Name $rgName -ErrorAction SilentlyContinue) {
        Write-Host "Resource Group: $rgName already exist" -f Yellow
    } else {
        Write-Host "Creating Resource Group: $rgName" -f Green
...
#Create VMs
    ...
    #Get VM Credentials from KeyVault
        $kvName = "TestVMKeyVault"
        $kv = Get-AzKeyVault -Name $kvName -ResourceGroupName $rgName
    ...
    $SrvJob = Start-Job -ScriptBlock {
        ...
        #Get VM Credentials from KeyVault
        $kvName = REad-Host -Prompt "Enter the name of the KeyVault in Resource Group: $($rgName))"
        $kv = Get-AzKeyVault -Name $kvName -ResourceGroupName $rgName
        $userName = Get-AzKeyVaultSecret -VaultName $kvName -Name "InstallerLocalAccountName" -AsPlainText
        $password = Get-AzKeyVaultSecret -VaultName $kvName -Name "InstallerLocalAccountSecret" -AsPlainText
        $vmCred = New-Object System.Management.Automation.PSCredential ($userName, (ConvertTo-SecureString $password -AsPlainText -Force))
...
New-VmsWithBastian -NewResourceName $name
...
$cleanUpJob = Start-Job -ScriptBlock {
    ...
    try {
        ...
        $nwr = Get-AzResourceGroup -ResourceGroupName 'NetworkWatcherRG' -ErrorAction SilentlyContinue
        if ($nwr) {
